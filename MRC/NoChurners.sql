...
SELECT DISTINCT ACT_ACCT_CD, DATE(DATE_TRUNC(MAX(CST_CHRN_DT), MONTH)) AS CHURN_DATE
   /* , CASE  
        WHEN C_CUST_AGE <= 3 THEN "3M"
        WHEN C_CUST_AGE > 3 AND C_CUST_AGE <= 6 THEN "3M-6M"
        WHEN C_CUST_AGE > 6 THEN "6M+"
    END AS TENURE*/
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D`
GROUP BY ACT_ACCT_CD--, TENURE
...
SELECT DATE_TRUNC(FECHA_EXTRACCION, MONTH) MONTH
    , MAX(FECHA_EXTRACCION) LAST_DATE_MONTH
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D`
...
,ACTIVE_MONTHS AS (
SELECT DISTINCT DATE_TRUNC(FECHA_EXTRACCION, MONTH) AS MONTH_ACT, ACT_ACCT_CD
   , CASE  
        WHEN C_CUST_AGE <= 3 THEN "3M"
        WHEN C_CUST_AGE > 3 AND C_CUST_AGE <= 6 THEN "3M-6M"
        WHEN C_CUST_AGE > 6 THEN "6M+"
    END AS TENURE
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D`
WHERE DATE_TRUNC(FECHA_EXTRACCION, MONTH) IN ('2021-07-01','2021-08-01','2021-09-01','2021-10-01')
)
, BASE AS (
SELECT M.MONTH_ORDER_ROW
    ...
    , ROUND(( COALESCE(SAFE_CAST(CRM.VO_FI_BILL_AMT_M0 AS FLOAT64),0) + CRM.BB_FI_BILL_AMT_M0 + CRM.TV_FI_BILL_AMT_M0),2) AS BILLING_TOTAL
FROM `gcp-bia-tmps-vtr-dev-01.gcp_temp_cr_dev_01.2022-02-16_FINAL_HISTORIC_CRM_FILE_2021_D` CRM
    ...
    INNER JOIN ACTIVE_MONTHS AM
        ON CRM.ACT_ACCT_CD=AM.ACT_ACCT_CD
    LEFT JOIN CHURN_MONTH_FILTER CH
        ON CRM.ACT_ACCT_CD = CH.ACT_ACCT_CD
    WHERE CH.ACT_ACCT_CD IS NULL
)
, PIVOT_VALS AS (
SELECT MONTH_ACT
    ,TENURE
...
SELECT PV.*
    ...
    , CASE 
        ...
        WHEN BILL_MONTH_1 > BILL_MONTH_2 THEN 'WENT DOWN'
        WHEN BILL_MONTH_1 = BILL_MONTH_2  THEN 'SAME'
        ELSE 'WENT UP'
    ...
    , CASE 
        ...
        WHEN BILL_MONTH_2 > BILL_MONTH_3 THEN 'WENT DOWN'
        WHEN BILL_MONTH_2 = BILL_MONTH_3  THEN 'SAME'
        ELSE 'WENT UP'
