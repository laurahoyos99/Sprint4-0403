...
SELECT DISTINCT EXTRACT(MONTH FROM d.MaxFecha) AS Mes, DistritoSinTildes, ACT_RGN_CD, COUNT(ACT_ACCT_CD) AS RegTot,
COUNT(CONTRATOSO) AS RegChurners, ROUND(COUNT(CONTRATOSO)/COUNT(ACT_ACCT_CD),2) AS ChurnRate
FROM CONTEOREGISTROS r INNER JOIN CONTEOCHURNERS c ON r.DistritoSinTildes=c.DistritoSinTildes
GROUP BY Mes,DistritoSinTildes, ACT_RGN_CD
ORDER BY Mes,DistritoSinTildes,ACT_RGN_CD
